/**
 * Wendet supabase/160_anni_management_finance.sql an.
 *
 *   node --env-file=.env.local scripts/apply-anni-management-finance.mjs
 */
...
const sqlPath = join(root, "supabase/160_anni_management_finance.sql");
...
async function verify() {
  ...
  if (jobsErr) {
    console.error("[verify jobs]", jobsErr.message);
    return false;
  }
  const { error: colErr } = await admin.from("profiles").select("id,is_management").limit(1);
  if (colErr) {
    console.error("[verify is_management]", colErr.message);
    return false;
  }
  console.log("[verify] anni_mgmt_jobs + profiles.is_management bereit");
  return true;
}
...
console.log("✓ Abrechnung Anni & Management Schema bereit");
