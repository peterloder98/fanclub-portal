/**
 * Wendet supabase/144_app_registration_status.sql an.
 * Benötigt SUPABASE_DB_PASSWORD in .env.local.
 *
 *   node --env-file=.env.local scripts/apply-app-registration-status.mjs
 */
...
const sqlPath = join(root, "supabase/144_app_registration_status.sql");
...
async function verify() {
  ...
  const { data, error } = await admin
    .from("profiles")
    .select("id,app_registration_status")
    .limit(1);
  if (error) {
    console.error("[verify]", error.message);
    return false;
  }
  console.log("[verify] sample row:", data?.[0] ?? null);
  return true;
}
...
console.log("✓ app_registration_status verfügbar");
