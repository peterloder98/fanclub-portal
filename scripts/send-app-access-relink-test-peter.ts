/**
 * Testmail der geplanten Relink-Kampagne — NUR an mail@peter-loder.de.
 * Kein Massenversand, keine Live-Vorlage überschreiben.
 *
 *   EMAIL_OUTBOUND_MODE=live npx --yes tsx --env-file=.env.local scripts/send-app-access-relink-test-peter.ts
 */
import { rotateAccountSetupToken } from "../src/lib/auth/account-setup-token";
import { loadDefaultMailSignature } from "../src/lib/email/default-mail-signature";
import {
...
import { emailPersonVars } from "../src/lib/email/salutation-block";
import { sendEmailViaAccount } from "../src/lib/smtp/send-via-account";

const TO = "mail@peter-loder.de";
const PETER_USER_ID = "1b70d88f-e28d-48f3-b3cb-646eaf06f19a";
const BASE = (
...
async function resolveSetupUrl(): Promise<{ url: string; note: string }> {
  try {
    const { setupUrl } = await rotateAccountSetupToken({
      email: TO,
      userId: PETER_USER_ID,
    });
    return { url: setupUrl, note: "echter Club-Setup-Token (145 ok)" };
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.warn("Kein echter Setup-Token (vermutlich SQL 145 fehlt):", msg);
    return {
      url: `${BASE}/setup-account`,
      note: `Platzhalter-URL — 145 fehlt (${msg})`,
    };
  }
}

async function main() {
  const { url: setupUrl, note } = await resolveSetupUrl();
  ...
  const text = `${fill(body_text, vars).trim()}\n\n${sig.text.trim()}`.trim();
  const html = wrapEmailDocument(
    appendSignatureToEmailHtml(fill(body_html, vars), sig.htmlBlock),
  );

  const result = await sendEmailViaAccount({
    to: TO,
    subject: `[TEST – bitte prüfen] ${subject}`,
    text,
    html,
