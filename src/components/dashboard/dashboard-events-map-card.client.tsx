...
export function DashboardEventsMapCard({ events }: { events: MapEvent[] }) {
  ...
  return (
    <>
      ...
      <Card className="hidden min-h-[240px] flex-1 flex-col overflow-hidden lg:flex">
        <CardContent className="flex h-full min-h-0 flex-1 flex-col p-1">
          <EventsMap events={events} minHeight={240} mapVariant="dashboard" fillHeight />
        </CardContent>
      </Card>
    </>
  );
