-- Event-Reiseinfos: strukturiert, für alle Mitglieder lesbar, nur Admins schreiben

alter table public.event_admin_notes
  add column if not exists travel_info jsonb not null default '{}'::jsonb;

-- Alte Textfelder in travel_info übernehmen
...
on public.event_admin_notes for select to authenticated
using (true);
