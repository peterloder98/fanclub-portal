-- Merchandise: Bestellungen, Reservierung, Einkaufshistorie, mehrere Buchungen
...
insert into public.merchandise_product_ledger_links (product_id, ledger_entry_id)
...
create index if not exists merchandise_stock_receipts_product_idx
  on public.merchandise_stock_receipts (product_id, created_at desc);
...
drop trigger if exists merchandise_orders_set_updated_at on public.merchandise_orders;
create trigger merchandise_orders_set_updated_at
...
-- Mitglieder: verkaufbare Artikel lesen
...
using (
  exists (
    select 1 from public.merchandise_products p
    where p.id = product_id and p.sale_price_cents > 0
  )
);
