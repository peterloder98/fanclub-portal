-- Komplette Reparatur / Nachholen der Vorgangsnummern (JJJJ-NNNN).
-- Kann ausgeführt werden, wenn 074 fehlgeschlagen oder nur teilweise gelaufen ist.
...
-- Schutz-Trigger vorübergehend entfernen (Backfill darf NULL → Nummer setzen)
...
-- Zähler zurücksetzen und aus bereits vergebenen Nummern ableiten (falls vorhanden)
delete from public.club_ledger_year_counters;

insert into public.club_ledger_year_counters (fiscal_year, last_seq)
select
  split_part(entry_number, '-', 1)::int as fiscal_year,
  max(split_part(entry_number, '-', 2)::int) as last_seq
from public.club_ledger_entries
where entry_number is not null
  and entry_number ~ '^\d{4}-\d{4}$'
group by 1;
